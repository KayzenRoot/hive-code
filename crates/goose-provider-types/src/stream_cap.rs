//! Wall-clock cap for one provider response.
//!
...
mod tests {
    ...
    async fn wedged_stream_errors_at_cap_without_retrying_transiently() {
        ...
        assert!(
            matches!(error, ProviderError::RequestFailed(ref msg) if msg.contains("maximum duration"))
        );
        assert!(!should_retry(
            &error,
    ...
    async fn zero_disables_cap() {
        ...
        assert!(
            tokio::time::timeout(Duration::from_secs(86_400), uncapped.next())
                .await
                .is_err()
        );
    }
